...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, onLogout, isAuthenticated, userType }) => {
    return (
        <>
            ...
            <div className="hidden md:flex fixed left-0 top-0 h-full w-64 bg-slate-900 flex-col shadow-2xl z-50 border-r border-slate-800">
                {/* Logo Area */}
                ...
                <div className="flex-1 w-full px-4 space-y-2 mt-4">
                    ...
                    >
                        ...
                        <span className="font-bold text-sm">Registration</span>
                    </button>
                    ...
                    >
                        ...
                        <span className="font-bold text-sm">Students List</span>
                    </button>
            ...
            <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-sm h-16 bg-slate-900/90 backdrop-blur-xl rounded-full shadow-2xl z-50 border border-slate-800 flex items-center justify-around px-2">
                <button
                    onClick={() => setActiveTab('form')}
                    className={`flex flex-col items-center justify-center transition-all duration-300 ${activeTab === 'form' ? 'text-blue-500' : 'text-slate-500'}`}
                >
                    <UserPlus size={24} className={activeTab === 'form' ? 'scale-110 drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]' : ''} />
                    <span className="text-[8px] font-bold uppercase tracking-tighter mt-1">Enroll</span>
                </button>

                <button
                    onClick={() => setActiveTab('list')}
                    className={`flex flex-col items-center justify-center transition-all duration-300 ${activeTab === 'list' ? 'text-blue-500' : 'text-slate-500'}`}
                >
                    <Users size={24} className={activeTab === 'list' ? 'scale-110 drop-shadow-[0_0_8px_rgba(59,130,246,0.5)]' : ''} />
                    <span className="text-[8px] font-bold uppercase tracking-tighter mt-1">List</span>
                </button>
                ...
                >
                    <LogOut size={22} />
                    <span className="text-[8px] font-bold uppercase tracking-tighter mt-1">Exit</span>
                </button>
            </div>
        </>
    );
...
const ConnectionStatus: React.FC<ConnectionStatusProps> = ({ compact }) => {
    ...
    if (loading && !status) return <div className="text-[10px] font-bold text-slate-600 px-4">...</div>;

    if (compact) {
        return (
            <div className={`w-3 h-3 rounded-full ${status?.connected ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]'}`} title={status?.connected ? 'Online' : 'Offline'}></div>
        );
    }
